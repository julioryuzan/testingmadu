...
async function _getExportRekap(p, signal) {
  ...
  garduRows.forEach(function(g) {
    ...
    merged.push({
      ...
      // ── Kolom master/nameplate trafo (untuk sheet Master Gardu) ──
      tipe:            g.tipe             || '',
      merekTrafo:      g.merek_trafo      || '',
      serialNumber:    g.serial_number    || '',
      arusPrimer:      g.arus_primer      != null ? String(g.arus_primer)      : '',
      arusSekunder:    g.arus_sekunder    != null ? String(g.arus_sekunder)    : '',
      tegPrimer:       g.tegangan_primer  != null ? String(g.tegangan_primer)  : '',
      tegSekunder:     g.tegangan_sekunder!= null ? String(g.tegangan_sekunder): '',
      frekuensi:       g.frekuensi_hz     != null ? String(g.frekuensi_hz)     : '',
      vektorGrup:      g.vektor_grup      || '',
      jenisOli:        g.jenis_oli        || '',
      volumeOli:       g.volume_oli_liter != null ? String(g.volume_oli_liter) : '',
      beratTotal:      g.berat_total_kg   != null ? String(g.berat_total_kg)   : '',
      tahunProduksi:   g.tahun_produksi   != null ? String(g.tahun_produksi)   : '',
      latitude:        g.latitude         || '',
      longitude:       g.longitude        || '',
      tglUkur:     tglUkur,
